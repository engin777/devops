...
```bash
helm create engin-chart
```

- See the files of engin-chart.
...
```bash
rm -rf engin-chart/templates/*
```

- Create a `configmap.yaml` file under `engin-chart/templates` folder with following content.

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: engin-chart-config
data:
  myvalue: "Hello World"
```

- Install the engin-chart.

```bash
helm install helm-demo engin-chart
```
...
```bash
kubectl get cm
kubectl describe cm engin-chart-config
```
...
- Let's create our own values and use it within the template. Update the `engin-chart/values.yaml` as below.

```yaml
course: DevOps
```

- Edit the engin-chart/templates/configmap.yaml as below.
...
metadata:
  name: engin-chart-config
data:
  myvalue: "engin-chart configmap example"
...
```bash
helm install --debug --dry-run mydryrun engin-chart
```

- Install the engin-chart.

```bash
helm install myvalue engin-chart
```
...
```bash
helm install --debug --dry-run setflag engin-chart --set course=AWS
```
...
```bash
helm install --debug --dry-run builtin-object engin-chart
```

- Let's try more examples to get more clarity. Update the `engin-chart/values.yaml` as below.
...
- Update the `engin-chart/templates/configmap.yaml` as below.
...
```bash
helm install --debug --dry-run morevalues engin-chart
```
...
- To add installation notes to your chart, simply create a `engin-chart/NOTES.txt` file. This file is plain text, but it is processed like a template and has all the normal template functions and objects available.

- Let's create a simple `NOTES.txt` file under `engin-chart/templates` folder.
...
```bash
helm install notes-demo engin-chart
```
...
```bash
helm repo add mylocalrepo http://44.203.223.134:8080
```
...
```bash
mkdir my-charts
mv engin-chart-0.1.0.tgz my-charts
helm repo index my-charts --url http://44.203.223.134:8080
```
...
entries:
  ...
  - apiVersion: v2
    ...
    type: application
    urls:
    - http://44.203.223.134:8080/engin-chart-0.1.0.tgz
    version: 0.1.0
...
```bash
cd my-charts
curl --data-binary "@engin-chart-0.1.0.tgz" http://44.203.223.134:8080/api/charts
```
...
- Let's see how to maintain the chart version. In `engin-chart/Chart.yaml`, set the `version` value to `0.1.1`and then package the chart.

```bash
helm package engin-chart
mv engin-chart-0.1.1.tgz my-charts
helm repo index my-charts --url http://44.203.223.134:8080
```
...
```bash
cd my-charts
curl --data-binary "@engin-chart-0.1.1.tgz" http://44.203.223.134:8080/api/charts
```
...
- In engin-chart/Chart.yaml, set the `version` value to `0.2.0`and push the chart.

```bash
cd
helm cm-push engin-chart mylocalrepo
```
...
```bash
helm cm-push engin-chart mylocalrepo --version="1.2.3"
```
...
```bash
helm cm-push engin-chart mylocalrepo --version="1.2.4"
```
...
helm repo update
helm search repo mylocalrepo
helm install from-local-repo mylocalrepo/engin-chart
```
...
```bash
helm cm-push engin-chart mylocalrepo --version="1.2.5"
helm repo update
```
...
```bash
helm upgrade from-local-repo mylocalrepo/engin-chart
```
...
```bash
helm upgrade from-local-repo mylocalrepo/engin-chart --version 1.2.4
```
